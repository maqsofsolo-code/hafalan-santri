...
export default function RekapNilaiUjianGuru({ data, loading, error, onRefresh }: Props) {
  ...
  const periodeOptions = useMemo(() => {
    ...
    data.forEach(item => {
      const key = item.kalender_id || 'tanpa-periode'
      const label = item.kalender?.nama || 'Tanpa Periode Kalender'
  ...
  const tipeOptions = useMemo(() => {
    return [...new Set(data.map(item => item.tipe).filter((value): value is string => Boolean(value)))]
  ...
  const santriOptions = useMemo(() => {
    const options = new Map<string, string>()
    data.forEach(item => {
      if (item.santri?.id) options.set(item.santri.id, item.santri.nama)
    })
    return [...options.entries()].sort((a, b) => a[1].localeCompare(b[1], 'id'))
  }, [data])
  ...
  const guruOptions = useMemo(() => {
    const options = new Map<string, string>()
    data.forEach(item => {
      if (item.guru?.id) options.set(item.guru.id, item.guru.nama)
    })
    return [...options.entries()].sort((a, b) => a[1].localeCompare(b[1], 'id'))
  }, [data])
  ...
  const hasil = useMemo(() => {
    ...
    return data.filter(item => {
      ...
      if (keyword) {
        const teks = `${item.santri?.nama || ''} ${item.guru?.nama || ''} ${labelKelas(item)}`.toLocaleLowerCase('id')
        if (!teks.includes(keyword)) return false
      }
      return true
    })
  }, [data, filterPeriode, filterTipe, filterKelas, filterSantri, filterGuru, pencarian])
    ...
    <div>
      <div className="rounded-2xl p-5 mb-5 text-white relative overflow-hidden shadow-lg"
        ...
        <div className="relative z-10">
          ...
          <p className="text-orange-200 text-sm mt-1">Satu baris mewakili satu nilai per segmen hafalan</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <div>
            ...
            <select value={filterKelas} onChange={event => setFilterKelas(event.target.value)} className={inputClass}>
              <option value="semua">Semua Kelas yang Diakses</option>
              {kelasOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1">Santri</label>
            <select value={filterSantri} onChange={event => setFilterSantri(event.target.value)} className={inputClass}>
              <option value="semua">Semua Santri</option>
              {santriOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
            </select>
          </div>
          <div>
            ...
            <select value={filterGuru} onChange={event => setFilterGuru(event.target.value)} className={inputClass}>
              <option value="semua">Semua Guru pada Hasil</option>
          ...
          <div>
            ...
            <input value={pencarian} onChange={event => setPencarian(event.target.value)}
              placeholder="Cari santri, guru, atau kelas..." className={inputClass} />
          </div>
        </div>
        <div className="flex flex-wrap items-center justify-between gap-2 mt-4">
          <p className="text-xs text-gray-500">{hasil.length} dari {data.length} nilai ditampilkan</p>
          <div className="flex gap-2">
            <button type="button" onClick={resetFilter} className="px-3 py-2 rounded-lg bg-gray-100 text-gray-600 text-xs font-semibold">
              Reset Filter
            </button>
            <button type="button" onClick={onRefresh} disabled={loading}
              className="px-3 py-2 rounded-lg bg-orange-100 text-orange-700 text-xs font-semibold disabled:opacity-50">
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[1320px] text-sm">
                <thead className="bg-orange-50 text-orange-900">
                  <tr>
                    {['Nama Santri', 'Kelas', 'Guru Penginput', 'Tanggal', 'Rentang Surah', 'Rentang Ayat', 'Teguran', 'Diberi Tahu', 'Lupa', 'Nilai', 'Aksi'].map(label => (
                      <th key={label} className="px-3 py-3 text-left font-semibold whitespace-nowrap">{label}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {hasil.map(item => (
                    <tr key={item.id} className="hover:bg-orange-50/40">
                      ...
                      <td className="px-3 py-3 text-gray-600">{namaSurah(item, 'mulai')} – {namaSurah(item, 'selesai')}</td>
                      <td className="px-3 py-3 text-gray-600 whitespace-nowrap">{item.ayat_mulai ?? '-'} – {item.ayat_selesai ?? '-'}</td>
                      <td className="px-3 py-3 text-center">{item.jumlah_tegur ?? 0}</td>
                      <td className="px-3 py-3 text-center">{item.jumlah_tahu_ayat ?? 0}</td>
                      <td className="px-3 py-3 text-center">{item.jumlah_lupa ?? 0}</td>
                      <td className="px-3 py-3 font-bold text-orange-700">{nilaiAman(item.nilai_akhir)}</td>
              ...
              <div key={item.id} className="bg-white rounded-2xl shadow border border-gray-100 p-4 overflow-hidden">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="font-bold text-gray-800 break-words">{item.santri?.nama || '-'}</div>
                    <div className="text-xs text-gray-500 mt-0.5 break-words">{labelKelas(item)}</div>
                    <div className="text-xs text-gray-500 break-words">Guru: {item.guru?.nama || '-'}</div>
                  </div>
                  <div className="text-2xl font-bold text-orange-700 flex-shrink-0">{nilaiAman(item.nilai_akhir)}</div>
                </div>
                <div className="mt-3 text-sm text-gray-600 space-y-1">
                  ...
                  <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-500">
                    <span>Tegur: {item.jumlah_tegur ?? 0}</span>
                    <span>Diberi tahu: {item.jumlah_tahu_ayat ?? 0}</span>
                    <span>Lupa: {item.jumlah_lupa ?? 0}</span>
                  </div>
                </div>
                <button type="button" onClick={() => setDetail(item)}
                  className="w-full mt-3 px-3 py-2 rounded-xl bg-orange-100 text-orange-700 text-sm font-semibold">
                  Detail
                </button>
              </div>
            ))}
      ...
      {detail && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={() => setDetail(null)}>
          <div className="bg-white w-full sm:max-w-2xl max-h-[92vh] overflow-y-auto rounded-t-3xl sm:rounded-2xl shadow-2xl"
            onClick={event => event.stopPropagation()}>
            <div className="sticky top-0 px-5 py-4 flex justify-between items-center text-white"
              style={{ background: 'linear-gradient(135deg, #7c2d12 0%, #ea580c 100%)' }}>
              <div>
                <h3 className="font-bold text-lg">Detail Nilai Ujian</h3>
                <p className="text-orange-200 text-xs">Satu nilai per segmen hafalan</p>
              </div>
              <button type="button" onClick={() => setDetail(null)} className="text-2xl leading-none" aria-label="Tutup detail">×</button>
            </div>
            <div className="p-5">
              <DetailRow label="Nama Santri" value={detail.santri?.nama || '-'} />
              <DetailRow label="Jenjang" value={labelJenjang(detail.santri?.jenjang)} />
              <DetailRow label="Kelas" value={labelKelas(detail)} />
              <DetailRow label="Guru Penginput" value={detail.guru?.nama || '-'} />
              <DetailRow label="Tanggal" value={formatTanggal(detail.tanggal)} />
              <DetailRow label="Kalender" value={detail.kalender?.nama || 'Tanpa Periode Kalender'} />
              <DetailRow label="Tipe Ujian" value={labelTipe(detail.tipe)} />
              <DetailRow label="Surah" value={`${namaSurah(detail, 'mulai')} – ${namaSurah(detail, 'selesai')}`} />
              <DetailRow label="Ayat" value={`${detail.ayat_mulai ?? '-'} – ${detail.ayat_selesai ?? '-'}`} />
              <DetailRow label="Jumlah Teguran" value={detail.jumlah_tegur ?? 0} />
              <DetailRow label="Diberi Tahu" value={detail.jumlah_tahu_ayat ?? 0} />
              <DetailRow label="Jumlah Lupa" value={detail.jumlah_lupa ?? 0} />
              <DetailRow label="Nilai Akhir" value={nilaiAman(detail.nilai_akhir)} />
              <DetailRow label="Catatan" value={detail.catatan || '-'} />
              <DetailRow label="Waktu Input" value={formatWaktu(detail.created_at)} />
              <div className="mt-5 p-3 rounded-xl bg-blue-50 border border-blue-200 text-blue-800 text-sm">
                Jika terdapat kesalahan input, silakan hubungi Admin.
              </div>
              <button type="button" onClick={() => setDetail(null)} className="w-full mt-4 py-3 rounded-xl bg-gray-100 text-gray-700 font-semibold">
                Tutup
              </button>
            </div>
          </div>
